"""
    ...
    publically for everyone to see the day after the finals. Students can go to a public bulletin and check their
    grades to see how they did on an exam and where their ranking stands. This kata, Student Rankings will have you,
    the professor automate the list making for the many classes you're in charge of!
...
def post_grades(students):
    ...
    for student in students:
        local_st_data: list = student.split(" - ")
        students_data.append((local_st_data[0], sum(map(lambda x: float(x), local_st_data[-1].split(" "))) / len(local_st_data[-1].split(" "))))
    students_data = sorted(students_data, key=lambda x: x[-1])[::-1]
