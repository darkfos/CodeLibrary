"""
    Write a function called which_postcode/whichPostcode that takes a string, and determines whether it represents a
    British or Slovakian postcode. If the string is a valid British postcode, return "GB". If it's a valid Slovakian
    postcode, return "SK". If the input is neither a valid British nor a valid Slovakian postcode, return "Not valid".

    Leading and trailing spaces should be ignored, but if there are spaces in wrong place in the middle of the postcode,
    then it is invalid.
...
def which_postcode(postcode):
    ...
    if len(postcode) == 6:
        if postcode[0:3].isdigit() and postcode[3] == " " and postcode[4:].isdigit():
            return "SK"

    postcode_gb = postcode.split(" ")
    if len(postcode_gb) == 2:
        if postcode_gb[0][0].isalpha() and len(postcode_gb[0]) >= 2:
            cnt_alpha, cnt_digits = sum(1 for i in postcode_gb[0] if i.isalpha()), sum(1 for i in postcode_gb[0] if i.isdigit())
            if (cnt_alpha >= 1 and cnt_alpha <= 2) and (cnt_digits >= 1 and cnt_digits <= 2):
                if postcode_gb[1][0].isdigit() and postcode_gb[1][1:].isalpha():
                    return "GB"
    return "Not valid"